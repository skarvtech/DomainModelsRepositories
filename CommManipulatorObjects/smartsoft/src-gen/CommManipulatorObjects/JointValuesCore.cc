...
{
	const char* JointValuesCore::getCompiledHash()
	{
		return CommManipulatorObjectsIDL::REPO_HASH;
	}
	
	void JointValuesCore::getAllHashValues(std::list<std::string> &hashes)
	{
		// get own hash value
		hashes.push_back(getCompiledHash());
	}
	
	void JointValuesCore::checkAllHashValues(std::list<std::string> &hashes)
	{
		...
		if (strcmp(getCompiledHash(), hashes.front().c_str()) != 0)
		{
			...
			std::cerr << "JointValuesCore hash" << std::endl;
	...
	#ifdef ENABLE_HASH
	size_t JointValuesCore::generateDataHash(const DATATYPE &data)
	{
		size_t seed = 0;
	...
	// default constructor
	JointValuesCore::JointValuesCore()
	:	idl_JointValues()
	{  
	...
	JointValuesCore::JointValuesCore(const DATATYPE &data)
	:	idl_JointValues(data)
	{  }

	JointValuesCore::~JointValuesCore()
	{  }
	
	void JointValuesCore::to_ostream(std::ostream &os) const
	{
	  os << "JointValues(";
	...
	void JointValuesCore::to_xml(std::ostream &os, const std::string &indent) const {
		size_t counter = 0;
	...
	void JointValuesCore::from_xml(std::istream &is) {
		size_t counter = 0;
	...
	/*
	void JointValuesCore::get(ACE_Message_Block *&msg) const
	{
		...
		// (see JointValuesACE.hh)
		cdr << idl_JointValues;
		
	#ifdef ENABLE_HASH
		ACE_CDR::ULong data_hash = generateDataHash(idl_JointValues);
		cdr << data_hash;
		// std::cout << "JointValuesCore: current data hash: " << data_hash << std::endl;
	#endif
	...
	void JointValuesCore::set(const ACE_Message_Block *msg)
	{
		...
		cdr >> data_hash;
		ACE_CDR::Long own_hash = generateDataHash(idl_JointValues);
		assert(data_hash == own_hash);
		// std::cout << "JointValuesCore: own data hash: " << own_hash << "; received data hash: " << data_hash << std::endl;
	#endif
	}
	*/
} /* namespace CommManipulatorObjects */
