...
std::string PTUCoordinationServiceCore::switchCi(const std::string& ciInstanceName, const std::string& componentName, const std::string& componentInstanceName, const std::string& service, const std::string& inString){
	...
	if(iter != ciInstanceMap.end()){
		...
		outString << "(error (unknown error))";
			...
			if(strcasecmp(service.c_str(), "ptuevent-activate") == 0 )
			{
				Smart::StatusCode status;
				...
				param = iter->second.pTUCoordinationServiceptueventEventHandlerCore->activateEventParam(input);
			...
			if(strcasecmp(service.c_str(), "ptuevent-deactivate") == 0)
			{
				Smart::StatusCode status;
				char *input  = (char *)NULL;
				char *pointer = (char *)NULL;
			...
			if(strcasecmp(service.c_str(), "ptupose") == 0 )
			{
				DomainPTU::CommPTUMoveRequest com;
				
				Smart::StatusCode status;
				com = iter->second.pTUCoordinationServiceptuposeSendHandler->handleSend(inString);
