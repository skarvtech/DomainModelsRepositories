...
{
	const char* CommDetectedMarkerCore::getCompiledHash()
	{
		return CommTrackingObjectsIDL::REPO_HASH;
	}
	
	void CommDetectedMarkerCore::getAllHashValues(std::list<std::string> &hashes)
	{
		// get own hash value
	...
	void CommDetectedMarkerCore::checkAllHashValues(std::list<std::string> &hashes)
	{
		...
		if (strcmp(getCompiledHash(), hashes.front().c_str()) != 0)
		{
			...
			std::cerr << "CommDetectedMarkerCore hash" << std::endl;
	...
	#ifdef ENABLE_HASH
	size_t CommDetectedMarkerCore::generateDataHash(const DATATYPE &data)
	{
		size_t seed = 0;
	...
	// default constructor
	CommDetectedMarkerCore::CommDetectedMarkerCore()
	:	idl_CommDetectedMarker()
	{  
		setId(0);
	...
	:	idl_CommDetectedMarker(data)
	{  }

	CommDetectedMarkerCore::~CommDetectedMarkerCore()
	{  }
	
	void CommDetectedMarkerCore::to_ostream(std::ostream &os) const
	{
	...
	void CommDetectedMarkerCore::to_xml(std::ostream &os, const std::string &indent) const {
	...
	/*
	void CommDetectedMarkerCore::get(ACE_Message_Block *&msg) const
	{
		...
		// (see CommDetectedMarkerACE.hh)
		cdr << idl_CommDetectedMarker;
		
	#ifdef ENABLE_HASH
		ACE_CDR::ULong data_hash = generateDataHash(idl_CommDetectedMarker);
		cdr << data_hash;
		// std::cout << "CommDetectedMarkerCore: current data hash: " << data_hash << std::endl;
	#endif
	...
	void CommDetectedMarkerCore::set(const ACE_Message_Block *msg)
	{
		...
		cdr >> data_hash;
		ACE_CDR::Long own_hash = generateDataHash(idl_CommDetectedMarker);
		assert(data_hash == own_hash);
		// std::cout << "CommDetectedMarkerCore: own data hash: " << own_hash << "; received data hash: " << data_hash << std::endl;
	#endif
	}
	*/
} /* namespace CommTrackingObjects */
