...
{
	const char* CommDetectedMarkerEventResultCore::getCompiledHash()
	{
		return CommTrackingObjectsIDL::REPO_HASH;
	}
	
	void CommDetectedMarkerEventResultCore::getAllHashValues(std::list<std::string> &hashes)
	{
		// get own hash value
	...
	void CommDetectedMarkerEventResultCore::checkAllHashValues(std::list<std::string> &hashes)
	{
		...
		if (strcmp(getCompiledHash(), hashes.front().c_str()) != 0)
		{
			...
			std::cerr << "CommDetectedMarkerEventResultCore hash" << std::endl;
	...
	#ifdef ENABLE_HASH
	size_t CommDetectedMarkerEventResultCore::generateDataHash(const DATATYPE &data)
	{
		size_t seed = 0;
	...
	:	idl_CommDetectedMarkerEventResult(data)
	{  }
	...
	/*
	void CommDetectedMarkerEventResultCore::get(ACE_Message_Block *&msg) const
	{
		...
		// (see CommDetectedMarkerEventResultACE.hh)
		cdr << idl_CommDetectedMarkerEventResult;
		
	#ifdef ENABLE_HASH
		ACE_CDR::ULong data_hash = generateDataHash(idl_CommDetectedMarkerEventResult);
		cdr << data_hash;
		// std::cout << "CommDetectedMarkerEventResultCore: current data hash: " << data_hash << std::endl;
	#endif
	...
	void CommDetectedMarkerEventResultCore::set(const ACE_Message_Block *msg)
	{
		...
		cdr >> data_hash;
		ACE_CDR::Long own_hash = generateDataHash(idl_CommDetectedMarkerEventResult);
		assert(data_hash == own_hash);
		// std::cout << "CommDetectedMarkerEventResultCore: own data hash: " << own_hash << "; received data hash: " << data_hash << std::endl;
	#endif
	}
	*/
} /* namespace CommTrackingObjects */
