...
{
	const char* CommObjectBeliefCore::getCompiledHash()
	{
		return CommObjectRecognitionObjectsIDL::REPO_HASH;
	}
	
	void CommObjectBeliefCore::getAllHashValues(std::list<std::string> &hashes)
	{
		// get own hash value
		hashes.push_back(getCompiledHash());
		// get hash value(s) for CommBasicObjects::CommPose3d(idl_CommObjectBelief.pose)
		CommBasicObjects::CommPose3d::getAllHashValues(hashes);
	}
	
	void CommObjectBeliefCore::checkAllHashValues(std::list<std::string> &hashes)
	{
		...
		if (strcmp(getCompiledHash(), hashes.front().c_str()) != 0)
		{
			...
			std::cerr << "CommObjectBeliefCore hash" << std::endl;
		...
		// check hash value(s) for CommBasicObjects::CommPose3d(idl_CommObjectBelief.pose)
		CommBasicObjects::CommPose3d::checkAllHashValues(hashes);
	}
	
	#ifdef ENABLE_HASH
	size_t CommObjectBeliefCore::generateDataHash(const DATATYPE &data)
	{
		size_t seed = 0;
	...
	// default constructor
	CommObjectBeliefCore::CommObjectBeliefCore()
	...
	:	idl_CommObjectBelief(data)
	{  }

	CommObjectBeliefCore::~CommObjectBeliefCore()
	{  }
	
	void CommObjectBeliefCore::to_ostream(std::ostream &os) const
	{
	  os << "CommObjectBelief(";
	  os << getType() << " ";
	  os << getProbability() << " ";
	...
	void CommObjectBeliefCore::to_xml(std::ostream &os, const std::string &indent) const {
		size_t counter = 0;
	...
	void CommObjectBeliefCore::from_xml(std::istream &is) {
		size_t counter = 0;
	...
	/*
	void CommObjectBeliefCore::get(ACE_Message_Block *&msg) const
	{
		...
		// (see CommObjectBeliefACE.hh)
		cdr << idl_CommObjectBelief;
		
	#ifdef ENABLE_HASH
		ACE_CDR::ULong data_hash = generateDataHash(idl_CommObjectBelief);
		cdr << data_hash;
		// std::cout << "CommObjectBeliefCore: current data hash: " << data_hash << std::endl;
	#endif
	...
	void CommObjectBeliefCore::set(const ACE_Message_Block *msg)
	{
		...
		cdr >> data_hash;
		ACE_CDR::Long own_hash = generateDataHash(idl_CommObjectBelief);
		assert(data_hash == own_hash);
		// std::cout << "CommObjectBeliefCore: own data hash: " << own_hash << "; received data hash: " << data_hash << std::endl;
	#endif
	}
	*/
} /* namespace CommObjectRecognitionObjects */
