...
#include <smartNumericCorrelationId.h>
 ...
 void ConveyerBeltCoordinationServiceCore::addNewModuleInstance(const std::string& name){
 	std::cout<<"addNewModuleInstance name:"<<name<<std::endl;
	ConveyerBeltCoordinationService mi;
	 ciInstanceMap.insert ( std::pair<std::string,ConveyerBeltCoordinationService>(name,mi));
 }
 
 int ConveyerBeltCoordinationServiceCore::initCiInstance(SmartACE::SmartComponent * component, const std::string& ciInstanceName, const std::map< std::string, CiConnection, ciLessLibC > &ciConnectionsMap){
 	std::map<std::string, ConveyerBeltCoordinationService, ciLessLibC>::iterator iter = ciInstanceMap.find(ciInstanceName);
...
std::string ConveyerBeltCoordinationServiceCore::switchCi(const std::string& ciInstanceName, const std::string& componentName, const std::string& componentInstanceName, const std::string& service, const std::string& inString){
	std::map<std::string, ConveyerBeltCoordinationService>::const_iterator iter = ciInstanceMap.find(ciInstanceName);
